...
- The build hook returns early when a build does not request code assets.
  It read the target architecture for the BLAKE3 build first, and that
  setting exists only in code-asset builds; any other build failed inside the
  hook. A new test runs the hook with no asset types requested.
